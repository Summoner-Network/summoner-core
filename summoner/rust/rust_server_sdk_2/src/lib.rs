...
// Our crate has two parts:
// - `logger` is a module we want everyone to use.
// - `server` is private to this file.
pub mod logger;
mod server;
...
/// Define a Python module named `rust_server_sdk_2`.
/// Inside, register the `start_tokio_server` function so Python can import it.
#[pymodule]
fn rust_server_sdk_2(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
